...
const Scroll = () => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      const walk = (x - startX) * 2; // multiply the movement
  ...
  return (
    <div
      className="overflow-x-auto flex space-x-4 p-4 cursor-grab mb-4  mt-4"
      style={{ ...scrollbarHideStyle, display: 'flex', overflowX: 'auto', padding: '16px' }}
      ref={carouselRef}
    ...
    >
      {images.map((imgSrc, index) => (
        <div
          key={index}
          className="flex-shrink-0 w-24 h-24 rounded-full overflow-hidden border-2 border-rosinha"
        >
          <img src={imgSrc} alt={`Story ${index + 1}`} className="border-rosinha w-full h-full object-cover" />
        </div>
      ))}
    </div>
  );
